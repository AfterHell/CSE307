	...
	while(change_count > 0) 
	{
		...
		for(row = 0, baseConsRow = 0; row < 9 ; row++) 
		{
			for(col = 0, baseConsCol = 0; col < 9 ; col++) 
			{
				...
				{	// if we have not already set thsi box in search see if it can change
					...
					//printf("%d%s%d\n", row, ", ", col);
					if((col %3) != 0) 
					{	// if not first col in 3x3 block, check constraint with box to left
					...
					if((col %3) != 2)
					{	// if not last col in 3x3 block, check constraint with box to right
						chkMask = pss->avail_mask[row][col+1];
					...
					if((row %3) != 0) 
					{	// if not top row in 3x3 block, check constraint with box above
					...
					if((row %3) != 2)
					{	// if not bottom row in 3x3 block, check constraint with box below
						chkMask = pss->avail_mask[row+1][col];
						resultMask = checkConstraint(constraints[baseConsRow+1][col], baseMask, chkMask);
						if(resultMask != 0) 
						{
					...
					//printf("%s%d\n", "baseMask = ", baseMask);
					if(baseMask == 0) 
					{	// not already set and no values available for box
						return -1;
					}
					pss->avail_mask[row][col] = baseMask; 
					if(totResult != 0) 
					{	// adjust counts for boxes in same row, col or 3x3 box
						for(i = 0; i < 9 ; i++) 
						{
				...
				if((col %3) != 2) 
				{	// advance to next row constraint if not last col in 3x3 box
					baseConsCol++;
				}
			}
			if((row % 3) != 2) 
			{	// if not last row of 3x3 box used 2 rows of constrsints, else 1
				baseConsRow += 2;
			} 
			else 
			{
...
int Solve(int level)
{
	...
	{	// for each candidate in chosen row, col or 3x3 box, get row& col to set
		//printf("%s\n", "hi");
		if(level == 80) 
		{	// if this is the last box to fill in we are done, set it and return 0 (success)
			pss->val_set[sd.test_row][sd.test_col] = sd.solve_val;
			return 0;
		} 
		else 
		{	// else copy current to next and try each possiblility
...
{int i = 0;
	int j = 0;
		for(i = 0; i < 9 ; i++) {
			for(j = 0; j < 9 ; j++) {
				printf("%d ", states[0].avail_mask[i][j]);
			}
			printf("\n");
		}printf("\n");
}
...
{int i = 0;
	int j = 0;
		for(i = 0; i < 9 ; i++) {
			for(j = 0; j < 9 ; j++) {
				printf("%d ", states[0].row_avail_counts[i][j]);
			}
			printf("\n");
		}printf("\n");
}
...
{int i = 0;
	int j = 0;
		for(i = 0; i < 9 ; i++) {
			for(j = 0; j < 9 ; j++) {
				printf("%d ", states[0].col_avail_counts[i][j]);
			}
			printf("\n");
		}printf("\n");
}
	...
	int k = 0;
		for(i = 0; i < 3 ; i++) 
		{
			for(j = 0; j < 3 ; j++) 
			{
				for(k = 0; k < 9 ; k++) 
				{
					printf("%d ", states[0].box_avail_counts[i][j][k]);
				}
			}
			printf("\n");
		}printf("\n");
